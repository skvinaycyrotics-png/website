...
export const fadeUpVariant = {
  ...
  show: { 
    opacity: 1, import * as React from 'react';
import Image from 'next/image';
import { LucideIcon } from 'lucide-react';
import { cn } from '@/lib/utils';
...
export function PageHero({
  ...
  return (
    <section
      className={cn(
        'relative w-full overflow-hidden pt-28 pb-16 sm:pt-36 sm:pb-24',
        hasImage ? 'bg-[#0a1628]' : 'bg-transparent',
        className
      )}
      {...props}
    >
      ...
      {!hasImage && (
        <div
          className={cn(
            'absolute inset-0 via-transparent to-transparent opacity-50',
            gradientClass,
            gradientFrom
          )}
        />
      ...
      <div className="container relative z-20 px-4 sm:px-6">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <h1
            className={cn(
              'font-headline text-4xl font-extrabold tracking-tight sm:text-5xl md:text-6xl lg:text-7xl',
          ...
          {description && (
            <p
              className={cn(
                'mx-auto mt-5 max-w-3xl text-base leading-relaxed sm:text-lg md:text-xl',
                hasImage
                  ? 'text-white/90'
                  : 'text-foreground/90'
              )}
